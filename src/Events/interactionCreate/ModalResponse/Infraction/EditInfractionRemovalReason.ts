...
export default async function (interaction: ModalSubmitInteraction, client: Client<true>, handler: CommandKit): Promise<undefined> {
    ...
    await UpdateEntity(InfractionSystem, { ActionID: custom_id, InfractionGuildID: interaction.guild.id }, { RemovalReason: interaction.fields.getTextInputValue("reason") });
    ...
    return void await interaction.followUp({ content: `Action ID [\`${custom_id}\`] removal reason has been edited in **${interaction.guild.name}**!`, ephemeral: true });
};
...
export default async function (interaction: ModalSubmitInteraction, client: Client<true>, handler: CommandKit): Promise<undefined> {
    if (!interaction.inCachedGuild() || !interaction.isModalSubmit() || interaction.customId !== "EditModerationAppealLink") return;
    ...
    await UpdateEntity(GuildModerationSetting, { Snowflake: interaction.guild.id }, { AppealLink: interaction.fields.getTextInputValue("appeal_link") === "{remove}" ? "" : interaction.fields.getTextInputValue("appeal_link") });

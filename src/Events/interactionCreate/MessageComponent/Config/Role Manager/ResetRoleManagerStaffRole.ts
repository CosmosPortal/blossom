...
export default async function (interaction: MessageComponentInteraction, client: Client<true>, handler: CommandKit): Promise<undefined> {
    if (!interaction.inCachedGuild() || !interaction.isButton() || !interaction.customId.startsWith("ResetRoleManagerStaffRole")) return;
    ...
    await UpdateEntity(RoleManager, { Snowflake: interaction.guild.id }, { [`StaffTeam${custom_id[1]}`]: 0 });

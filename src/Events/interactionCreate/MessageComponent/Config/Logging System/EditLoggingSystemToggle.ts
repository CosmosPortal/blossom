...
export default async function (interaction: MessageComponentInteraction, client: Client<true>, handler: CommandKit): Promise<undefined> {
    if (!interaction.inCachedGuild() || !interaction.isStringSelectMenu() || interaction.customId !== "EditLoggingSystemToggle") return;
    ...
    for (const value of interaction.values) {
        ...
        const current_value = logging_setting[value as keyof LoggingSetting] ?? false;

        await UpdateEntity(LoggingSetting, { Snowflake: interaction.guild.id }, { [value]: !current_value });
    };

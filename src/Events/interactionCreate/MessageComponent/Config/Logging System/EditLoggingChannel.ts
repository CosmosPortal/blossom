...
export default async function (interaction: MessageComponentInteraction, client: Client<true>, handler: CommandKit): Promise<undefined> {
    if (!interaction.inCachedGuild() || !interaction.isChannelSelectMenu() || !interaction.customId.startsWith("EditLoggingChannel")) return;
    ...
    await UpdateEntity(LoggingSetting, { Snowflake: interaction.guild.id }, { [custom_id]: interaction.values[0] });

...
export default async function (interaction: MessageComponentInteraction, client: Client<true>, handler: CommandKit): Promise<undefined> {
    if (!interaction.inCachedGuild() || !interaction.isChannelSelectMenu() || !interaction.customId.startsWith("EditReportChannel")) return;
    ...
    await UpdateEntity(ReportSetting, { Snowflake: interaction.guild.id }, { [custom_id]: interaction.values[0] });

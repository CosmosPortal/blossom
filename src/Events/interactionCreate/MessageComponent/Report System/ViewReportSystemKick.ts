...
export default async function (interaction: MessageComponentInteraction, client: Client<true>, handler: CommandKit): Promise<undefined> {
    if (!interaction.inCachedGuild() || !interaction.isButton() || interaction.customId !== "ViewReportSystemKick") return;
    ...
    const modal_response = new ModalBuilder({
        custom_id: "TriggerReportSystemKick",
        title: "Report System | Kick"
    })
    .CreateTextInput({
        custom_id: "reason",
        label: "The reason for the kick",
        style: TextInputStyle.Paragraph,
        max_length: 250,
        min_length: 5,
        placeholder: "Please provide a reason for the kick",

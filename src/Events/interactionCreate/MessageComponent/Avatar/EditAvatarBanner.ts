...
export default async function (interaction: MessageComponentInteraction, client: Client<true>, handler: CommandKit): Promise<undefined> {
    if (!interaction.inCachedGuild() || !interaction.isStringSelectMenu() || !(interaction.customId.startsWith("EditAvatarBannerExtension") || interaction.customId.startsWith("EditAvatarBannerSize"))) return;
    ...
    const extension = custom_id[0] === "EditAvatarBannerExtension" ? interaction.values[0] : custom_id[2] as AvatarExtension;
    const size = Number(custom_id[0] === "EditAvatarBannerSize" ? interaction.values[0] : custom_id[3]) as AvatarSize;
    ...
    const banner_url = user.bannerURL({
        size: size,

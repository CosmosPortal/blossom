...
import { ActionType, Blossom, FindGuildInfractions, FindOrCreateEntity, GuildID, Sentry, type InfractionType } from "../../../../Core";
...
export default async function (interaction: MessageComponentInteraction, client: Client<true>, handler: CommandKit): Promise<undefined> {
    ...
    const infractions = await FindGuildInfractions(interaction.guild.id, type, true);
    if (!infractions) return void await Blossom.CreateInteractionError(interaction, `${interaction.guild.name} doesn't have any ${ActionType[type].toLowerCase()} action IDs that exist.`);
    ...
    const format = {
        ...
        total_infractions: guild_id.InfractionCreation,
    ...
    return void await interaction.followUp({ content: `Exported infraction data belonging to ${interaction.guild.name}. Staff data are marked as "anonymous" for privacy.`, files: [file_one] });
};